...
int main()
{
	int i,count,totaal,d1,d2,d3,d4,degree,pg,dimaut,h11;
...
*/
	...
	printf("d1 d2 d3 d4 d #f dim_aut pg h11 h02\n");
	d1 = 1;
	while (d1 <= MAX) {
	  d2 = d1;
	  while (d2 <= MAX) {
	    ...
	    while (d3 <= MAX) {
		  ...
		  while (degree <= 10*MAX) {
			...
			dimaut = dim_aut(d1, d2, d3, d4);
			h11 = hilbert_function(d1, d2, d3, d4,
				degree, 2*degree-d1-d2-d3-d4);
			printf("%d %d %d %d\n", dimaut, pg, h11, pg);
		      }
		    }
		    degree++;
